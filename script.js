...
document.addEventListener('DOMContentLoaded', () => {
    const animateElements = document.querySelectorAll('.service-card, .founder-card, .contact-item');
    ...
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            stats.forEach(stat => {
                ...
                } else if (text.includes('2-10')) {
                    stat.textContent = '2-10';
                }
            });
...
window.addEventListener('scroll', () => {
    ...
    const heroContent = document.querySelector('.hero-container'); // Apply to content, not whole hero
    if (heroContent) {
        const rate = scrolled * 0.3; // Adjust rate for desired effect
...
// Add styles for particle canvas
